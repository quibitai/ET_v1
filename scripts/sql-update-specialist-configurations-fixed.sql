-- Phase 2.1: Specialist Configuration Update SQL (FIXED for JSON type)
-- Execute this in Supabase SQL Editor to complete specialist tool configurations
-- Generated: 2025-01-29 - Fixed for json type (not jsonb)

-- First, let's see current configurations
SELECT name, json_array_length(default_tools) as current_tool_count FROM specialists ORDER BY name;
...
SELECT 
  ...
  CASE 
    WHEN name = 'Echo Tango' AND json_array_length(default_tools) = 48 THEN '✅ COMPLETE'
    WHEN name = 'General Chat' AND json_array_length(default_tools) = 25 THEN '✅ COMPLETE'  
    WHEN name = 'Test' AND json_array_length(default_tools) = 12 THEN '✅ COMPLETE'
    ELSE '❌ NEEDS_UPDATE'
  END as status
FROM specialists 
ORDER BY name;
