...
async def get_gmail_thread_content(
    ...
    # Implement response size limit to prevent hangs (max 50KB)
    MAX_RESPONSE_SIZE = 50000
    if len(content_text) > MAX_RESPONSE_SIZE:
        truncated_text = content_text[:MAX_RESPONSE_SIZE]
        # Find last complete message boundary to avoid cutting off mid-message
        last_message_boundary = truncated_text.rfind("=== Message")
        if last_message_boundary > 0:
            truncated_text = truncated_text[:last_message_boundary]
        
        truncated_text += f"\n\n[TRUNCATED: Response too large ({len(content_text):,} chars). Showing first {len(truncated_text):,} characters. Thread has {len(messages)} total messages.]"
        logger.warning(f"[get_gmail_thread_content] Response truncated: {len(content_text):,} chars -> {len(truncated_text):,} chars")
