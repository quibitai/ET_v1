...
export const prompts: Record<string, MCPPrompt> = {
  'task-summary': {
    name: 'task-summary',
    description:
  ...
  'project-status-report': {
    ...
    description:
      'Generate a comprehensive project status report with task completion, team workload, and timeline analysis',
    arguments: [
      {
        name: 'project_id',
  ...
  'workflow-planning': {
    ...
    description:
      'Create a structured workflow plan for complex multi-step Asana operations',
    arguments: [
      {
  ...
  'team-productivity-analysis': {
    ...
    description:
      'Analyze team productivity patterns and provide insights based on task completion and assignment data',
    arguments: [
      {
        name: 'team_id',
      ...
      },
      {
        name: 'time_period',
        description:
          'Time period for analysis (e.g., "last_week", "last_month", "last_quarter")',
        required: false,
      },
    ],
  },
...
export function createPromptHandler(client: AsanaClientWrapper) {
  return {
    listPrompts: async (): Promise<{ prompts: MCPPrompt[] }> => {
...
async function generateTaskSummaryPrompt(
  ...
  try {
    ...
    const task = await client.getTask(
      task_id,
      'name,notes,completed,assignee,assignee.name,due_on,created_at,modified_at,custom_fields,custom_fields.name,custom_fields.display_value,projects,projects.name,tags,tags.name,parent,parent.name,subtasks,subtasks.name,subtasks.completed',
    );

    // Fetch task stories/comments
    const stories = await client.getTaskStories(
      task_id,
      'text,created_at,created_by,created_by.name,type',
    );
...
- **Project(s)**: ${task.projects?.map((p: any) => p.name).join(', ') || 'No projects'}
- **Tags**: ${task.tags?.map((t: any) => t.name).join(', ') || 'No tags'}
...
${
  task.custom_fields?.length > 0
    ? task.custom_fields
        .map((cf: any) => `- **${cf.name}**: ${cf.display_value || 'Not set'}`)
        .join('\n')
    : 'No custom fields'
}
...
${
  task.subtasks?.length > 0
    ? task.subtasks
        .map((st: any) => `- [${st.completed ? 'x' : ' '}] ${st.name}`)
        .join('\n')
    : 'No subtasks'
}
...
${
  stories?.length > 0
    ? stories
        ...
        .map(
          (story: any) =>
            `**${new Date(story.created_at).toLocaleDateString()}** - ${story.created_by?.name || 'System'}: ${story.text || story.type}`,
        )
        .join('\n\n')
    : 'No recent activity'
}
...
async function generateProjectStatusReportPrompt(
  client: AsanaClientWrapper,
  args: any,
) {
  ...
  try {
    ...
    const hierarchy = await client.getProjectHierarchy(project_id, {
      ...
      opt_fields_project:
        'name,created_at,due_on,notes,owner,owner.name,team,team.name,archived',
      opt_fields_tasks:
        'name,completed,assignee,assignee.name,due_on,created_at,modified_at',
...
${section.tasks
  .map(
    (task: any) =>
      `- [${task.completed ? 'x' : ' '}] ${task.name} ${task.assignee?.name ? `(${task.assignee.name})` : '(Unassigned)'} ${task.due_on ? `- Due: ${task.due_on}` : ''}`,
  )
  .join('\n')}
`,
  )
...
${
  statuses?.length > 0
    ? statuses
        .map(
          (status: any) =>
            `**${new Date(status.created_at).toLocaleDateString()}** (${status.color || 'neutral'}) - ${status.created_by?.name || 'Unknown'}: ${status.text}`,
        )
        .join('\n\n')
...
Format the report in a professional, stakeholder-ready format with clear sections and actionable insights.
    ...
    throw new Error(
      `Failed to generate project status report prompt: ${error.message}`,
    );
  }
}

async function generateWorkflowPlanningPrompt(
  client: AsanaClientWrapper,
  args: any,
) {
  ...
  if (!workflow_description) {
    throw new Error(
      'workflow_description is required for workflow-planning prompt',
    );
  }
  ...
  try {
    ...
    let workspaceInfo = '';
    if (workspace_id) {
      const teams = await client.getTeamsForWorkspace(
        workspace_id,
        'name,description',
      );
      ...
      workspaceInfo = `
## Workspace Context
- **Teams Available**: ${teams.map((t: any) => t.name).join(', ')}
- **Team Members**: ${users
        .slice(0, 20)
        .map((u: any) => u.name)
        .join(
...
Please format your response as a clear, actionable workflow plan that could be executed either manually or through automation.
    ...
    throw new Error(
      `Failed to generate workflow planning prompt: ${error.message}`,
    );
  }
}

async function generateTeamProductivityAnalysisPrompt(
  client: AsanaClientWrapper,
  args: any,
) {
  ...
  if (!team_id) {
    throw new Error(
      'team_id is required for team-productivity-analysis prompt',
    );
  }
...
Please format your analysis in a clear, data-driven report suitable for team leads and management review.
    ...
    throw new Error(
      `Failed to generate team productivity analysis prompt: ${error.message}`,
    );
  }
}
